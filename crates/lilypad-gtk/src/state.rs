...
use lilypad_core::session_store::{ActiveRecord, SessionStore};
...
pub struct AppState {
    ...
    /// The durable session ledger. Starts unavailable and is opened by the primary instance in
    /// `app::build_window`, so a second launch (which only forwards activation over D-Bus) never
    /// touches it.
...
mod tests {
    ...
    fn logout_clears_account_caches_and_rejects_old_work() {
        let state = AppState::new(auth("alice"), ProcessMapConfig::default());
    ...
    fn switching_accounts_replaces_detection_preferences() {
        ...
        let state = AppState::new(auth("alice"), old_map);
...
impl AppState {
    ...
    pub fn change_account(&self, auth: AuthConfig) -> Result<(), String> {
        let process_map = if auth.token.is_some() {
            ProcessMapConfig::load_from(&lilypad_core::config::process_map_path_for_auth(&auth))
        } else {
            ProcessMapConfig::default()
        };
        auth.save_to(&lilypad_core::config::auth_config_path())
    ...
    pub fn refresh_installed_games(&self) {
        ...
        let steam_root = lilypad_core::steam::find_steam_root();
        ...
        let fingerprint = lilypad_core::local_games::install_locations_fingerprint(steam_root.as_deref(), &watched_dirs);
        let mut games = steam_root
            .as_deref()
            .map(lilypad_core::steam::scan_installed_games)
            .unwrap_or_default();
    ...
    pub fn refresh_installed_games_if_changed(&self) {
        let watched: Vec<String> = self
            ...
            .collect();
        let steam_root = lilypad_core::steam::find_steam_root();
        let fingerprint = lilypad_core::local_games::install_locations_fingerprint(steam_root.as_deref(), &watched);
