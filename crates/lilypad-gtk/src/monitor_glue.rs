...
use lilypad_core::api::ApiFailure;
use lilypad_core::config::{self, ProcessMapping};
use lilypad_core::ledger_session::now_secs;
use lilypad_core::monitor::{run_poll_loop, UnmappedSessionStart};
use lilypad_core::session_store::Completion;
...
///
/// Every durable write happens on the monitor thread *before* the event is sent: a GTK channel
/// message must never be the only record that a session started or ended.
#[derive(Debug, Clone)]
...
fn begin_session(state: &AppState, process_name: &str, mapping: &ProcessMapping) -> Option<String> {
...
fn backfill_exe_path(state: &AppState, mapping: &ProcessMapping, pid: u32) {
...
pub fn finish_session(state: &AppState, process_name: &str) -> Option<Option<String>> {
    // Already ended synchronously by the force-stop tray action. The slot is left alone: it may
    // already hold a different game that started after the force-stop.
...
fn begin_unmapped(state: &AppState, start: &UnmappedSessionStart) {
    ...
    lilypad_core::ledger_session::spawn_ledger_heartbeat(
        store.ledger(),
        Some(id),
...
fn finish_unmapped(
    ...
    let total = match id {
        ...
        None => state.account().and_then(|account| {
            store.record_new_game(&account, appid, title, exe_name, crate::session_flow::round_hours(duration_secs), replay_of)
        }),
    };
    match total {
        Some(total) => {
...
mod tests {
    ...
    use lilypad_core::session_ledger::SessionLedger;
    use lilypad_core::session_store::SessionStore;
    ...
    fn state(name: &str) -> AppState {
        let state = AppState::new(auth(name), ProcessMapConfig::default());
    ...
    fn a_force_stopped_session_is_not_presented_again_when_the_process_exits() {
        let state = state("alice");
        let id = begin_session(&state, "game", &mapping()).unwrap();
        // What the tray's force-stop does, synchronously.
    ...
    fn a_session_is_submittable_only_by_the_account_that_started_it() {
        ...
        assert!(crate::session_flow::may_submit(&state, Some(&id), alice.as_ref()));
        let bob = state.store().account(&auth("bob"));
        assert!(!crate::session_flow::may_submit(&state, Some(&id), bob.as_ref()));
        assert!(!crate::session_flow::may_submit(&state, Some(&id), None));
    }

    #[test]
    fn logged_out_play_is_tracked_in_memory_without_a_record() {
        let state = AppState::new(AuthConfig::default(), ProcessMapConfig::default());
